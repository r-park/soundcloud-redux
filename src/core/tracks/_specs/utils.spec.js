...
describe('tracks', () => {
  describe('utils', () => {
    ...
    describe('streamUrl()', () => {
      it('should return stream url with soundcloud client id', () => {
        const url = 'https://api.soundcloud.com/tracks/21857834/stream';
        const expectedUrl = `${url}?${CLIENT_ID_PARAM}`;
    ...
    describe('trackImageUrl()', () => {
      ...
      it('should transform artwork url to point to 500 pixel version', () => {
        const track = {artwork_url: artworkUrl}; // eslint-disable-line camelcase
        expect(trackImageUrl(track)).toBe(expectedArtworkUrl);
      });

      it('should use user avatar url if artwork url is not available', () => {
        const track = {user: {avatar_url: avatarUrl}}; // eslint-disable-line camelcase
